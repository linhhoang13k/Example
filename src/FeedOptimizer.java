...
public class FeedOptimizer {
  ...
  FeedEntry getEvents(int h, int s, int eventTime, FeedEntry fe, List<Event> listEvents, int index) {
    ...
    String key = eventTime + "-" + h + "-" + (index + 1);
    ...
    if ((e.time + s) >= eventTime && (h - e.height) >= 0) { // event fits on time and height...
      fe.addEvent(e);
...
//		System.out.println("\t" + key + ":::" + fe.toString() + " vs " + temporalFeedEntry.toString());
    // COMPARE events
    // if scores are equal get
    if (fe.score == temporalFeedEntry.score && fe.ids.size() == temporalFeedEntry.ids.size()) {
      fe = (fe.ids.peek() > temporalFeedEntry.ids.peek()) ? temporalFeedEntry : fe;
    } else if (fe.score == temporalFeedEntry.score) {
      fe = (fe.ids.size() > temporalFeedEntry.ids.size()) ? temporalFeedEntry : fe;
    } else {
      fe = (temporalFeedEntry.score > fe.score) ? temporalFeedEntry : fe;
    }
//		System.out.println("\t\t" + key + "::: winner " + fe.toString());
