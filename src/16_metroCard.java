...
int[] metroCard(int lastNumberOfDays) {
    ...
    ArrayList<Integer> possibleDaysList = new ArrayList<Integer>();
    if(lastNumberOfDays == 28 || lastNumberOfDays == 30){
        possibleDaysList.add(31);
    }
    else if(lastNumberOfDays == 31){
        possibleDaysList.addAll(Arrays.asList(28,30,31));
    }
    
    int[] possibleDaysArray = possibleDaysList.stream().mapToInt(i->i).toArray();
    return possibleDaysArray;
}